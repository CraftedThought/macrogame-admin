...
export const DataProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const duplicateCustomMicrogame = async (variant: CustomMicrogame): Promise<void> => {
    ...
    // We must copy files to the new ID's folder so they persist if the original is deleted.
    ...
    const copyPromises = Object.entries(variant.skinData).map(async ([key, fileData]) => {
        try {
            ...
            // Update the data to point to the new file
            newSkinData[key] = { url: newUrl, fileName: fileData.fileName };
        } catch (e) {
            console.error(`Failed to copy asset ${key}:`, e);
            // Fallback: Keep the old URL if copy fails (prevents breaking)
        }
    });
