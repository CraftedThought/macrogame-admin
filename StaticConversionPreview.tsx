...
import { MacrogameChrome } from '../ui/MacrogameChrome'; 
...
export const StaticConversionPreview: React.FC<StaticConversionPreviewProps> = ({ 
    ...
    const barebonesStyle = {
        ...
        gameArea: {
            ...
            flexDirection: 'column' as const,
            alignItems: 'center',       
            justifyContent: 'flex-start' // Vertically align to top
        }
    };
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginLeft: '1rem', borderLeft: '1px solid #ddd', paddingLeft: '1rem' }}>
                        ...
                        <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                            <input
                                ...
                                min="0"
                                max="100"
                                placeholder="60" // Update: Added placeholder
                                value={previewWidth || ''}
                                onChange={(e) => {
                                    const val = e.target.value;
                                    // 1. Handle Empty
                                    if (val === '') {
                                        onPreviewWidthChange(0);
                                        return;
                                    }
                                ...
                                onBlur={(e) => {
                                    let val = parseInt(e.target.value) || 60; // Default to 60 on blur if empty/invalid
                                    if (val < 20 && val !== 0) val = 20;
                                    if (val > 100) val = 100;
                                    onPreviewWidthChange(val);
                                }}
                                style={{
                                    width: '100px',
                                    padding: '2px 4px',
