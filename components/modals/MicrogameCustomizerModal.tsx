...
export const MicrogameCustomizerModal: React.FC<MicrogameCustomizerModalProps> = ({ microgame, existingVariant, onClose, onSave }) => {
    ...
    const handleSave = async () => {
        ...
        setIsLoading(true); // Keep this to disable the button
        ...
        try {
            ...
            // 3. Dismiss and Success
            notifications.dismiss(loadingToast);
            // Note: onSave in DataContext might send its own success toast (we haven't silenced that one yet), 
            // but having it here ensures consistency if we silence DataContext later. 
            // For now, this might double-toast, but it's safer to be explicit.
