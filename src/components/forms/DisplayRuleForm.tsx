...
interface DisplayRuleFormProps {
  ...
  // We now receive the pre-selected popups from the parent modal
  selectedPopups: Popup[];
}

export const DisplayRuleForm: React.FC<DisplayRuleFormProps> = ({ index, onRemove, selectedPopups }) => {
  const { register, control, watch, setValue } = useFormContext();
  const { macrogames, allMicrogames } = useData();
  const [isExpanded, setIsExpanded] = useState(true);

  const ruleName = watch(`displayRules.${index}.name`, `Rule #${index + 1}`);
  const popupWeights = watch(`displayRules.${index}.popups`, []);
            ...
            <p style={styles.descriptionText}>The following delivery methods will be rotated based on the weights you assign.</p>
             <div style={{maxHeight: '200px', overflowY: 'auto', border: '1px solid #ccc', borderRadius: '6px', marginTop: '0.5rem'}}>
              {selectedPopups.map(popup => {
                  ...
                  const currentPopupWeight = popupWeights.find((p: any) => p.popupId === popup.id)?.weight ?? 100;
                  ...
                  return (
                      <div key={popup.id} style={{...styles.rewardItem, backgroundColor: hasIssues ? '#f9f9f9' : 'white'}}>
                          ...
                          <div style={{display: 'flex', alignItems: 'center', gap: '0.5rem'}}>
                              <label>Weight:</label>
                              <input type="number" value={currentPopupWeight} onChange={(e) => handlePopupWeightChange(popup.id, e.target.value)} style={{...styles.pointsInput, width: '70px'}} />
                          </div>
                      </div>
                  );
              })}
