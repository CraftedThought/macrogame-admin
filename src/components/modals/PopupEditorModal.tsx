...
export const PopupEditorModal: React.FC<PopupEditorModalProps> = ({ isOpen, onClose, popup, onSave, macrogames }) => {
    ...
    const handleSave = () => {
        if (!popup) return;
        if (!macrogameId) { alert('Please select a macrogame for the popup.'); return; }
        const selectedMacrogame = macrogames.find(mg => mg.id === macrogameId);
        const updatedData: Partial<Popup> = { name, macrogameId, macrogameName: selectedMacrogame?.name || '', skinId, title, subtitle, colorScheme, };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Edit Popup" footer={modalFooter} size="small">
            <div>
