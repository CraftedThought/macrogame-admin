...
import baseStyles from './Tablet.module.css';
import blackStyles from './Tablet-black.module.css';

// SVG icons for buttons, with colors that work on light/dark backgrounds
...
const TabletSkin: React.FC<Props> = ({ children, isMuted, onClose, onMute, title, subtitle, colorScheme }) => {
    const isBlack = colorScheme === 'black';
    const colorStyles = isBlack ? blackStyles : {};
    ...
    return (
        <div className={`${baseStyles.container} ${colorStyles.container || ''}`}>
            ...
            <img src={`/assets/svgs/tablet-${isBlack ? 'black' : 'white'}-camera-lens.svg`} className={baseStyles.cameraLens} alt="Camera" />
            <div className={`${baseStyles.homeButton} ${colorStyles.homeButton || ''}`} />
            ...
            <button onClick={onMute} className={`${baseStyles.muteButton} ${colorStyles.muteButton || ''}`} aria-label="Mute"><img src={isMuted ? muteIcon : unmuteIcon} alt="Mute" /></button>
            <button onClick={onClose} className={`${baseStyles.exitButton} ${colorStyles.exitButton || ''}`} aria-label="Close"><img src={exitIcon} alt="Exit" /></button>
        </div>
    );
};
