...
const algoliaIndices = {
  macrogames: algoliaClient.initIndex("macrogames"),
  campaigns: algoliaClient.initIndex("campaigns"),
  // --- REFACTOR: Rename 'popups' to 'deliveryContainers' ---
  deliveryContainers: algoliaClient.initIndex("deliveryContainers"),
  conversionMethods: algoliaClient.initIndex("conversionMethods"),
  conversionScreens: algoliaClient.initIndex("conversionScreens"),
  customMicrogames: algoliaClient.initIndex("customMicrogames"),
  microgames: algoliaClient.initIndex("microgames"), // <-- ADDED
};
...
const transformConversionMethodDoc = async (
  ...
  // Add health checks here if needed in the future
  return {
    ...method,
    status: { code: "ok", message: "" }, // Default status
  };
};
...
const transformMacrogameDoc = async (
  ...
  if (macrogame.conversionScreenId) {
    ...
    tags.push("no_conversion_methods");
  }

  // Add tag for music filter
  if (!macrogame.config.backgroundMusicUrl) {
    tags.push("music_url_null");
  }
  ...
  if (macrogame.introScreen.enabled) {
    totalDuration += macrogame.introScreen.duration;
  }
  if (macrogame.promoScreen?.enabled) {
    totalDuration += macrogame.promoScreen.duration || 0;
  }

  const screenTimePerGame = (() => {
    switch (macrogame.config.screenFlowType) {
      case "Separate":
        return (
          macrogame.config.titleScreenDuration / 1000 +
          macrogame.config.controlsScreenDuration / 1000
        );
      case "Combined":
        return macrogame.config.titleScreenDuration / 1000;
      default:
        return 0;
    }
  })();

  const flowLength = (macrogame.flow || []).reduce(
    (sum: number, flowItem: any) => {
      const microgame = allMicrogames.find(
        (mg: any) => mg.id === flowItem.microgameId,
      ) as any;
      return sum + (microgame?.length || 0) + screenTimePerGame;
    },
    0,
  );

  totalDuration += flowLength;

  // --- 5. Get Mechanic Types ---
  const mechanicTypes = [
    ...new Set(
      (macrogame.flow || [])
        .map((flowItem: any) => {
          ...
          return microgame?.mechanicType || null;
        })
        .filter(Boolean),
    ),
  ];

  // --- 6. Algolia Object Assembly ---
  return {
    //...macrogame, // Don't spread the whole doc, just what we need
    objectID: macrogame.objectID, // Will be added by the caller
    id: macrogame.id,
    name: macrogame.name,
    category: macrogame.category,
    subcategory: macrogame.subcategory,
    conversionGoal: macrogame.conversionGoal,
    gameplayExperience: macrogame.gameplayExperience,
    numGames: (macrogame.flow || []).length,
    hasCustomGames: variantIdList.length > 0 ? 1 : 0,
    conversionScreenId: macrogame.conversionScreenId || null,
    isFavorite: macrogame.isFavorite || false,
    duration: totalDuration,
    status: newStatus,
    musicUrl: macrogame.config.backgroundMusicUrl || null,
    conversionMethodTypes: conversionMethodTypes,
    mechanicTypes: mechanicTypes,
    "introScreen.enabled": (macrogame.introScreen?.enabled || false) ? 1 : 0,
    "promoScreen.enabled": (macrogame.promoScreen?.enabled || false) ? 1 : 0,
    _tags: tags,
    // Store these on the Algolia record, but they might not be needed for filtering
