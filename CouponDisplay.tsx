...
export const CouponDisplay: React.FC<CouponDisplayProps> = ({ 
  ...
  const codeBoxStyle: React.CSSProperties = {
    borderStyle: style?.strokeStyle || 'dashed',
    borderWidth: `${safeVal(style?.strokeWidth, 0)}px`,
    borderColor: style?.strokeColor || '#cfc33a',
    backgroundColor: style?.backgroundColor || '#1a1a1a',
    boxShadow: `0 10px 25px rgba(0,0,0,${safeVal(style?.boxShadowOpacity, 0) / 100})`,
    
    paddingTop: `${safeVal(style?.paddingTop, 0)}px`,
    paddingBottom: `${safeVal(style?.paddingBottom, 0)}px`,
    paddingLeft: `${safeVal(style?.paddingX, 0)}px`,
    paddingRight: `${safeVal(style?.paddingX, 0)}px`,
    ...
    alignItems: 'center',
    gap: `${safeVal(style?.spacing, 0)}px`,
  };
  ...
  return (
    <div style={containerStyle}>
      <div style={codeBoxStyle}>
        {method.headline && (
            <div className="coupon-content-wrapper ql-editor" style={{ color: style?.textColor || '#fff', textAlign: 'left', width: '100%' }}>
